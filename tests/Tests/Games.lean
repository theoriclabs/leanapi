...
structure Env where
  ...
  rt : Runtime
  svc : Service
  /-- The LeanDB-program service's connections, when it is the one under test. -/
  dc : Option LeanApi.DbConns := none

/-- Which implementation of the game routes is under test. -/
inductive Impl where
  | native  -- `App/Service.lean` over the scoped repository
  | dbapi   -- `DbApi.lean`: LeanDB programs (LAPI-05)
  deriving BEq

def Impl.name : Impl → String
  | .native => "native"
  | .dbapi => "LeanDB programs"

def Env.close (e : Env) : IO Unit := do
  if let some dc := e.dc then dc.close
  e.rt.close

def openService (impl : Impl) (path : System.FilePath) (readers : Nat := 2) : IO Env := do
  let rt ← Runtime.open path readers
  let dummy ← LeanCrypto.Password.hash "dummy" fast
  match impl with
  | .native => return { path, rt, svc := service rt.repo dummy (fun _ => pure ()) fast }
  | .dbapi =>
    let dc ← LeanApi.DbConns.open path PrivateGames.Storage.schema readers
    return { path, rt, svc := PrivateGames.DbApi.service dc rt.repo dummy (fun _ => pure ()) fast, dc := some dc }

def freshEnvFor (impl : Impl) (name : String) : IO Env := do
...
def runWith (impl : Impl) : TestM Unit := do
  let env ← freshEnvFor impl s!"games-{impl.name.replace " " "-"}"
  let svc := env.svc
  let (aliceId, alice) ← signup svc "alice"
  let (bobId, bob) ← signup svc "bob"
  let (_, eve) ← signup svc "eve"

  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}accounts (unproved routes)" do
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}open, read, list" do
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}§9.3: unauthorized vs missing ids" do
    let gid := 1
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}moves, If-Match, stale revision" do
    let gid := 1
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}keyed idempotence" do
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}§9.3: simultaneous moves" do
  ...
  if impl == .native then section_ "§9.3: revocation between admission and commit" do
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}stored row that fails validation" do
  ...
  section_ s!"{if impl == .native then "" else "[LeanDB programs] "}§9.3: restart after commit, retry returns the receipt" do
    ...
    let env2 ← openService impl env.path 1
    let svc2 := env2.svc
  ...
  if impl == .native then section_ "LAPI-04: typed schema symbols" do
  ...
  if impl == .native then section_ "LAPI-04: a v1 instance migrates on open" do
    let dir : System.FilePath := ".lake/test-db"
