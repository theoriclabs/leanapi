...
| Rows written for new and changed games are `Checked` from the domain proofs (`preserved_openGame`, `decide_valid`), with no runtime check; the row mapping round-trips on them | **Proved** | `PrivateGames.Storage.GameRow.ofGame_invariant`, `PrivateGames.Storage.GameRow.toGame_ofGame`, `PrivateGames.Storage.decide_participants`, `PrivateGames.PlayerId.make_lt`, `PrivateGames.Storage.pid_lt` |
