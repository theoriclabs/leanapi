...
| LeanDB's `GameRow` invariant (checked on every read and every write, LDB-16) is exactly `Valid` of the mapped game, for any id | **Proved** | `PrivateGames.Storage.GameRow.invariant_iff`, `PrivateGames.Storage.GameRow.Invariant_iff`, `PrivateGames.Storage.GameRow.valid_id_irrel`, `PrivateGames.Valid.holdsB_iff` |
| Rows written for new and changed games are `Checked` from the domain proofs (`preserved_openGame`, `decide_valid`), with no runtime check; player ids carry their range bound (`PlayerId.lt`), so the row mapping round-trips on every game | **Proved** | `PrivateGames.Storage.GameRow.ofGame_invariant`, `PrivateGames.Storage.GameRow.toGame_ofGame`, `PrivateGames.Storage.decide_participants`, `PrivateGames.Game.bounded`, `PrivateGames.Storage.pid_pref` |
