...
/-- The store as a system. It starts empty; the counter may start anywhere. -/
def sys : Sys where
  World := L.World
  Req := L.Req
...
/-- **The lift.** Per-entity obligations give the system invariant. -/
theorem allOf_inductive {I : L.Entity → Prop} (h : L.Preserves I) : Inductive L.sys (L.AllOf I) := by
  refine ⟨fun w hw x hx => by simp [sys] at hw; simp [hw] at hx, fun _ r w hw => ?_⟩
  cases r with
  | create c =>
...
theorem fresh_inductive (h : L.IdLaws) : Inductive L.sys L.Fresh := by
  refine ⟨fun w hw x hx => by simp [sys] at hw; simp [hw] at hx, fun _ r w hw => ?_⟩
  cases r with
  | create c =>
...
theorem unique_rel (h : L.IdLaws) : InductiveRel L.sys L.Fresh L.UniqueIds := by
  refine ⟨fun w hw => by simp [sys] at hw; simp [UniqueIds, hw], fun _ r w hf hu => ?_⟩
  cases r with
  | create c =>
